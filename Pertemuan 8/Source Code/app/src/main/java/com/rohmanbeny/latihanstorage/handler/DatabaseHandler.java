package com.rohmanbeny.latihanstorage.handler;
...
//    }
    ...
    public void addBuku(Post post) {
        ...
        try {
            long id = addOrUpdateBook(post.buku);
    ...
    private long addOrUpdateBook(BukuModel buku) {
        SQLiteDatabase db = this.getWritableDatabase();
        ...
        try {
            ContentValues values = new ContentValues();
            values.put(KEY_BOOK_JUDUL, buku.judul);

            int rows = db.update(TABLE_BOOKS, values, KEY_BOOK_ID + " = ?", new String[]{String.valueOf(buku.judul)});
            ...
            if (rows == 1) {
                ...
                Cursor cursor = db.rawQuery(bukuSelectQuery, new String[]{String.valueOf(buku.judul)});
        ...
        try {
            if (cursor.moveToFirst()) {
                do {
                    ...
                    newBuku.judul =  cursor.getString(cursor.getColumnIndex(KEY_BOOK_JUDUL));


                    Post newPost = new Post();
                    newPost.buku = newBuku;
    ...
    //Update data
    public int updatePost(BukuModel book) {
        SQLiteDatabase db = this.getWritableDatabase();

        ContentValues values = new ContentValues();
        values.put(KEY_BOOK_JUDUL, book.judul);

        return db.update(TABLE_BOOKS, values, KEY_BOOK_JUDUL + " = ?",
                new String[] { String.valueOf(book.judul) });
    }
